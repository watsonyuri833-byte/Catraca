    ...
    for bloco in blocos:
      ...
      if problema:
        ...
        passos_padrao = [{
            "passo": 1,
            "texto": (
                solucao_texto if solucao_texto else "Não informada"
            ),
            "anexo": None,
        }]
        ...
        dados = {
            "sistema": (
                sistema if sistema in LISTA_SISTEMA else "Outro Sistema"
            ),
...
def upload_arquivo_unico(file):
  ...
  try:
    ...
    url_res = (
        ...
        else supabase.storage.from_("anexos_evidencias").get_public_url(
            file_name
        )
    )
      ...
      if url_file.lower().endswith(extensoes_imagem):
        try:
          st.image(
              url_file,
              width=450,
              caption=f"Imagem do Problema {idx_prob + 1}: {nome_exibicao}",
          )
        except Exception:
          st.markdown(
          ...
          if url_file.lower().endswith(extensoes_imagem):
            try:
              st.image(
                  ...
                  width=450,
                  caption=(
                      f"Evidência {idx_f + 1} do Passo {num_passo}:"
                      f" {nome_exibicao}"
                  ),
              )
            except Exception:
              st.markdown(
                  f"📥 Baixar Arquivo {idx_f + 1} do Passo {num_passo}:"
                  f" [**{nome_exibicao}**]({url_file})"
              )
          else:
            st.markdown(
                f"📄 Arquivo {idx_f + 1} do Passo {num_passo}:"
                f" [**{nome_exibicao}**]({url_file})"
            )
      st.markdown("")
  else:
  ...
  if df_filtered.empty:
    ...
    if ocor_id_selecionado:
      ...
      with st.container(border=True):
        ...
        texto_pos = (
            f"👍 Funcionou ({v_pos})" + (" ✅" if user_voto == "pos" else "")
        )
        texto_neg = (
            f"👎 Não funcionou ({v_neg})"
        ...
        if st.session_state.user_role == "Admin":
          ...
          with st.expander(f"✏️ Editar Relato Finalizado #{ocor_id}"):
            with st.form(key=f"form_edit_{ocor_id}"):
              ...
              for p_num in range(1, 4):
                ...
                if urls_existentes:
                  st.markdown(
                      "📷 *Imagens atuais (desmarque para excluir):*"
                  )
              ...
              if st.form_submit_button("💾 Salvar Alterações"):
                ...
                if atualizar_ocorrencia_db(
                  ...
                  st.toast(
                      f"Tratativa #{ocor_id} atualizada com sucesso!",
                      icon="✅",
                  )
                  st.rerun()
          ...
          if st.button(
            ...
            if sucesso:
              st.toast(
                  f"Tratativa #{ocor_id} excluída com sucesso!", icon="🗑️"
              )
              st.rerun()
  ...
  with st.form("form_novo", clear_on_submit=True):
    ...
    for p_idx in range(1, 4):
      ...
      if txt_p.strip():
        ...
        passos_novos_lista.append(
            {"passo": p_idx, "texto": txt_p.strip(), "anexo": url_anexo_p}
        )
    ...
    if st.form_submit_button("💾 Salvar Mapeamento no Banco"):
      if in_prob and in_motivo and passos_novos_lista:
        ...
        url_anexo_prob = (
            upload_multiplos_arquivos(in_files_prob)
            if in_files_prob
            else None
        )
        autor_reg = (
